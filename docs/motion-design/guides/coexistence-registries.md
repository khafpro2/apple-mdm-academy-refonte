...
| Dossier de production complet d’une vidéo pilote (script, storyboard, captures, HeyGen, sous-titres, quiz) | `docs/motion-design/pilots/*.md` — orchestré via `src/lib/video-*` |
| Sous-titres français d’une vidéo publiée | Fichier statique `public/videos/captions/<slug>.fr.vtt`, sinon généré à la volée par `/api/videos/[slug]/captions` depuis `src/lib/video-transcripts.ts` (`lib/video/captions.ts`) |
