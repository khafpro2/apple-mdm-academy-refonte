 ...
 * Optional JSON report (gitignored): npm run audit:motion-assets -- --json
 */
...
function main(): void {
  ...
  const duplicateIds = issues.filter(
    (i) => i.code === "id-duplicate" || i.code === "scene-id-duplicate"
  ).length;
  ...
  if (jsonMode) {
    ...
    const outputPath = path.join(reportsDir, "motion-assets-audit.json");
